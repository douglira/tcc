							...
							<div class="card-body d-flex-column justify-content-center">				
								...
								<template v-if="productItem">
									<div class="container-product-item">
										...
										<img :alt="productItem.thumbnail.name" :src="productItem.thumbnail.urlPath"/>
										<div class="info_container-product-item">
											<div class="info_content-product-item">
												<p class="text-dark">{{ productItem.title }}</p>
												<small class="text-secondary">Relevância: {{ productItem.relevance }}</small>
												<br/>
												<small class="text-muted">Visualizações: {{ productItem.viewsCount }}</small>
											</div>
											<div class="price_content-product-item">
												<p class="text-success">R$ {{ productItem.marketPrice.toFixed(2) }}</p>
												<small class="text-muted">
													Flutuação (R$)
													<br/>
													{{ productItem.maxPrice.toFixed(2) }} - {{ productItem.minPrice.toFixed(2) }}
												</small>
											</div>
										</div>
									</div>
