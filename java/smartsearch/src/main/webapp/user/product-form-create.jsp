					...
					<div class="card border-light mb-3 mb-sm-3 mb-md-3" >
						<div class="card-body">
							...
							<p class="text-muted">
								...
								conste na nossa base de dados resultará na criação de um novo anúncio em nosso sistema.
									...
									<div class="list-group autocomplete-list" v-if="productsPredict && productsPredict.length">
										<template v-for="(predictProduct, index) in productsPredict">
											<a
												...
												<div>
													<img :alt="predictProduct.thumbnail.name" :src="predictProduct.thumbnail.urlPath" />
													<span>{{ predictProduct.title }}</span>
												</div>
												<small class="badge badge-pill badge-info text-light">{{ predictProduct.relevance }}</small>
											</a>
										</template>
									</div>
								</div>
						...
						<div class="card-body d-flex-column justify-content-center">
							...
							<template v-if="productItemPreview">
								<div class="container-product-item">
									<button type="button" class="close text-danger" aria-label="Close" @click="onClickRemoveProductItem">
										<span aria-hidden="true">&times;</span>
									</button>
									<img :alt="productItemPreview.thumbnail.name" :src="productItemPreview.thumbnail.urlPath"/>
									<div class="info_container-product-item">
										<div class="info_content-product-item">
											<p class="text-dark">{{ productItemPreview.title }}</p>
											<small class="text-secondary">Relevância: {{ productItemPreview.relevance }}</small>
											<br/>
											<small class="text-muted">Visualizações: {{ productItemPreview.viewsCount }}</small>
										</div>
										<div class="price_content-product-item">
											<p class="text-success">R$ {{ productItemPreview.marketPrice.toFixed(2) }}</p>
											<small class="text-muted">
												Flutuação (R$)
												<br/>
												{{ productItemPreview.maxPrice.toFixed(2) }} - {{ productItemPreview.minPrice.toFixed(2) }}
											</small>
										</div>
									</div>
								</div>
							...
							<template v-else>
								<p class="text-muted align-self-start">Nenhum anúncio vinculado.</p>
							</template>
						</div>
					</div>
				</div>
